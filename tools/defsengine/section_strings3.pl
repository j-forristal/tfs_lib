...
# NOTE: strings3 preserves order at cost of potential storage size; use strings2 if order doesn't matter
...
while(<IN>){
	...
	# SPECIAL: sort keys marker
	if( m/^#[ \t]*SORTKEYS/ ){
		push @DATA_ORDERED_KEYS, sort @current_keys;
		@current_keys = ();
		next;
	}
	...
	# save the key order
	#push @DATA_ORDERED_KEYS, $st;
	push @current_keys, $st;
}

push @DATA_ORDERED_KEYS, @current_keys if( ~~@current_keys > 0 );
