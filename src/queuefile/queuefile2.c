...
int TFQF_Push( TFQF_QueueFile_t *qf, uint8_t *msg, uint16_t len )
{
	...
	MEMCPY( &page[ sizeof(TFQF_Message_t) ], msg, amount );
...
int TFQF_Pop( TFQF_QueueFile_t *qf, uint8_t *buffer, uint16_t *len )
{
	...
	MEMCPY( msg_hash, m->hash, sizeof(m->hash) );
...
static int _new( TFQF_QueueFile_t *qf, uint8_t page[TFQF_PAGE_SIZE] )
{
	...
	MEMCPY( p0->nonce_check, hash, sizeof(p0->nonce_check) );
...
ASSERT( qf->fd != -1 );
	...
	MEMCPY( qf->hmac_key, hmac_key, TFQF_HMAC_KEY_SIZE );
